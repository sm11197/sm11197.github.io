...
judgments_online[consultancy_condition,] %>%
  ...
  summarise(
    ...
    p_aka_accuracy = (x_aka_num_correct / n),
    var_prop = (p_aka_accuracy * (1 - p_aka_accuracy)) / (n - 1)
  ) %>% group_by(Final_Setting) %>% summarise(avg_var_mean = mean(var_mean, na.rm = T),
               avg_var_prop = mean(var_prop, na.rm = T))
judgments_online[consultancy_condition,] %>%
  group_by(Question) %>%
  summarise(
...
judgments_online$`Reward penalty 0.5` <- log2(judgments_online$`Final probability correct`) - 0.5*
...
debater_turns %>%
  ...
  labs(y = "Total Quote Length in a Debate/Consultancy (unique tokens)", x = "Setting")+
  theme_minimal()
pairwise.wilcox.test(filtered$quote_length, filtered$Final_Setting)
...
pairwise.wilcox.test(debater_turns$`Max judge rounds by room`, debater_turns$Final_Setting)
...
debater_turns %>%
  ...
  labs(y = "Rounds", x = "Setting")+
  theme_minimal()
pairwise.wilcox.test(filtered$`Max judge rounds by room`, filtered$Final_Setting)
...
all_turns = turns.merge(
        debates[["Room name", "Honest debater", "Dishonest debater", "Question", "Article ID",
                 "Speed annotator accuracy","Untimed annotator context","Untimed annotator context bins","Is offline","Final_Setting", "Setting"]],
        how="left",
        on="Room name",
    )
```
...
strat <- strat %>%
  mutate(`Max judge rounds bin` = ifelse(`Max judge rounds by room` > 5, "6+", as.character(`Max judge rounds by room`))) %>%
...
strat %>%
  ...
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, fill = `Max judge rounds bin`, group = `Max judge rounds bin`, color = NULL), alpha = 0.25) +
